...
func ActionPullRequests(cmd *cobra.Command, opts PullRequestOpts) carapace.Action {
	return carapace.ActionCallback(func(args []string) carapace.Action {
		...
		return GraphQlAction(cmd, fmt.Sprintf(`repository(owner: $owner, name: $repo){ pullRequests(first: 100, states: %v) { nodes { number, title, mergeStateStatus } } }`, opts.states()), &queryResult, func() carapace.Action {
